...
for (i in 1:length(occ.sps)){
  ...
  sp.occ <- sp.file
  me <- maxent(Abiotic10, sp.occ, path=paste("/home/mshcheglovitova/PsittaJBI/Modelos Plantas 10 arc2/", splist[i], sep=""))
  r <- predict(me,Abiotic10)
  writeRaster(r,filename=paste("~/PsittaJBI/Bioticas Logistic 10 arc2/", splist[i], ".grd", sep=""), overwrite=T)
  tmp <- i
  print(tmp)
}
